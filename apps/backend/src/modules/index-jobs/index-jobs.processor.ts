...
export class IndexJobsProcessor extends WorkerHost {
  ...
  async process(queueJob: Job<IndexJobQueuePayload>) {
    ...
    try {
      ...
      if (error instanceof IndexJobCanceledError || await this.jobs.isCanceled(job.id)) {
        return;
      }
  ...
  private async parseDocument(indexJobId: string, document: { id: string; storagePath: string | null; mimeType: string | null; metadata: Prisma.JsonValue }) {
    if (!document.storagePath) throw new Error('Document has no uploaded file');
    await this.jobs.markRunning(indexJobId, '正在解析原始文件', 15);
    await this.prisma.knowledgeDocument.update({ where: { id: document.id }, data: { status: 'parsing' } });
    ...
    await this.prisma.knowledgeDocument.update({
      ...
      data: {
        ...
        metadata: this.mergeMetadata(document.metadata, {
          extractedAt: new Date().toISOString(), extractedText: text, extractedTextLength: text.length,
        }),
      },
    });
    await this.jobs.markSucceeded(indexJobId, { documentId: document.id, extractedTextLength: text.length });
  }

  private async indexDocument(indexJobId: string, document: { id: string; storagePath: string | null; mimeType: string | null; metadata: Prisma.JsonValue }) {
    const metadata = this.asRecord(document.metadata);
    const extractedText = typeof metadata.extractedText === 'string'
    ...
    await this.prisma.knowledgeDocumentChunk.deleteMany({ where: { documentId: document.id } });
    ...
    for (const [index, chunk] of chunks.entries()) {
      await this.jobs.markProgress(indexJobId, '正在写入索引数据', Math.min(95, 80 + Math.round(((index + 1) / chunks.length) * 15)));
    ...
    await this.prisma.knowledgeDocument.update({
      ...
      data: {
        status: 'indexed',
        metadata: this.mergeMetadata(document.metadata, {
          chunkCount: chunks.length, extractedText, extractedTextLength: extractedText.length,
          indexedAt: new Date().toISOString(),
        }),
      },
    });
    await this.jobs.markSucceeded(indexJobId, {
      chunkCount: chunks.length, documentId: document.id, embeddedCount: vectors.length,
    });
  }

  private async deleteDocument(indexJobId: string, document: { id: string; storagePath: string | null }) {
    await this.jobs.markRunning(indexJobId, '正在删除文档和索引数据', 25);
    const deletedChunks = await this.prisma.knowledgeDocumentChunk.count({ where: { documentId: document.id } });
    ...
    await this.jobs.markSucceeded(indexJobId, { deletedChunks, deletedFile, documentId: document.id });
  }

  private async rebuildAll(parentJobId: string) {
    await this.jobs.markRunning(parentJobId, '正在获取待重建文档', 5);
    const documents = await this.prisma.knowledgeDocument.findMany({ select: { id: true }, orderBy: { createdAt: 'asc' } });
    if (!documents.length) {
    ...
    for (const [index, document] of documents.entries()) {
      ...
      await this.jobs.markProgress(parentJobId, '正在重建文档索引', Math.min(95, Math.round(((index + 1) / documents.length) * 95)));
    }
    await this.jobs.markSucceeded(parentJobId, { documentCount: documents.length });
  }

  private async embedInBatches(texts: string[]) {
    const vectors: number[][] = [];
    for (let index = 0; index < texts.length; index += this.config.embeddingBatchSize) {
      vectors.push(...await this.embeddings.embed(texts.slice(index, index + this.config.embeddingBatchSize)));
    }
    return vectors;
  }

  private mergeMetadata(metadata: Prisma.JsonValue, patch: Record<string, unknown>): Prisma.InputJsonValue {
    return { ...this.asRecord(metadata), ...patch } as Prisma.InputJsonObject;
  }
